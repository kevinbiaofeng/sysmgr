						...
						<form id="search-form" method="post">
							<table class="form-table" border="0" cellpadding="1" cellspacing="2">
								<tr>
									...
									<td class="form-editor">
										<input id="tagid" name="tagid" class="easyui-combobox" editable="false" data-options="panelHeight:'auto', valueField:'id',textField:'text',
											data:[{id: 0,text: '普通'},{id: 1,text: '人气'}]" />
									</td>
				...
				<div class="panel-body" >
					...
					<div id="tb" style="display:none" class="opt-button-float">
						<div class="group-button opt-button-float">
							...
							<button class="layui-btn layui-btn-sm layui-btn-normal" id="add" name="add" onclick="addMenu()">
					...
					<table id="datagrid1" class="easyui-datagrid" title="充值配置信息"
						...
						<thead>
							<tr>
								...
								<th data-options="field:'configname',align:'center',fixed:false,width:30">配置名称</th>
								<th data-options="field:'wechat',align:'center',fixed:false,width:30">微信号</th>
								<th data-options="field:'nickname',align:'center',fixed:false,width:30">微信昵称</th>
								<th data-options="field:'tagidDesc',align:'center',fixed:false,width:30">标签类型</th>
								...
								<th data-options="field:'collectdate',align:'center',fixed:false,width:40,formatter:function(value,row,index){return $.dateFormatter(value)}">配置时间 </th>
	...
	<script type="text/javascript">
		...
		$(function($){
			...
			$("#add").bind("click",function(){
				$('#dialog').dialog({
					title: '新增在线充值',
					href: '${pageContext.request.contextPath}/invest/online/toOnlineAddPage',
					width: 800,
					height: 400,
					closed: false,
					cache: false,
		...
		function search(){
			...
			$('#datagrid1').datagrid({
				url:'${pageContext.request.contextPath}/invest/online/queryOnLineWithPage',
				pageNumber:1,
				queryParams : {
		...
		function openEditWindown(id){
			$('#dialog').dialog({
				title: '编辑在线充值',
				href: '${pageContext.request.contextPath}/invest/online/toOnlineEditPage',
		...
		function removeByids(){
			...
			$.messager.confirm('确认',"确定要删除选中的记录吗",function(r){
				if (r){
					...
					//提交到后台
					$.ajax({
						url: "${pageContext.request.contextPath}/invest/online/removeOnLineByIds?configids="+nmArr,
						type: 'DELETE',
						//dataType: 'json',
						//data: data,
