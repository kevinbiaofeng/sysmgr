		...
		<div class="row">
			<div class="col-lg-12">
				<div class="panel-body" >
					...
					<table id="datagrid1" class="easyui-datagrid" title="费用比例"
						...
						<thead>
							<tr>
								...
								<th data-options="field:'scorecountDouble',align:'center',fixed:false,width:30,formatter:function(value,row,index){return $.numberFormatter(value)}">注册赠送金币数</th>
								<th data-options="field:'upgradescorecountDouble',align:'center',fixed:false,width:30,formatter:function(value,row,index){return $.numberFormatter(value)}">升级赠送金币数</th>
								<th data-options="field:'visitorscorecountDouble',align:'center',fixed:false,width:30,formatter:function(value,row,index){return $.numberFormatter(value)}">游客赠送金币数</th>
								<th data-options="field:'platformtypeDesc',align:'center',fixed:false,width:30">平台类型</th>
								<th data-options="field:'registwebsite',align:'center',fixed:false,width:40">注册站点</th>
								<th data-options="field:'withdrawratio',align:'center',fixed:false,width:30">提现比率(‰)</th>
								<th data-options="field:'withdrawlowerlimit',align:'center',fixed:false,width:30">提现手续费下限</th>
								<th data-options="field:'withdrawupperlimit',align:'center',fixed:false,width:30">提现手续费上限</th>
								<th data-options="field:'withdrawscorelowerlimit',align:'center',fixed:false,width:30">提现金额下限</th>
								<th data-options="field:'withdrawscoreupperlimit',align:'center',fixed:false,width:30">提现金额上限</th>
								<th data-options="field:'reservescorelowerlimit',align:'center',fixed:false,width:30">保底金额</th>
	...
	<script type="text/javascript">
		$(function($){
			...
			$("#add").bind("click",function(){
				$('#dialog').dialog({
					title: '注册赠送',
					href: '${pageContext.request.contextPath}/account/reggive/toReggiveAddPage',
					width: 800,
					height: 400,
					closed: false,
					cache: false,
		...
		function search(){
			$('#datagrid1').datagrid({
				url:'${pageContext.request.contextPath}/account/reggive/queryReggiveWithPage',
				pageNumber:1,
				view: myview,
				loadMsg: "数据加载中，请稍后！",
				emptyMsg: "未搜索到相关数据！"
		...
		function openEditWindown(id){
			$('#dialog').dialog({
				title: '注册赠送',
				href: '${pageContext.request.contextPath}/account/reggive/toReggiveEditPage',
		...
		function deleteReggiveByids(){
			...
			$.messager.confirm('确认',"确定要删除选中的记录吗",function(r){
				if (r){
					...
					//提交到后台
					$.ajax({
						url: "${pageContext.request.contextPath}/account/reggive/removeReggiveByIds?configids="+ids,
						type: 'DELETE',
						//dataType: 'json',
						//data: data,
