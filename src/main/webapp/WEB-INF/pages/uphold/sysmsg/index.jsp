								...
								<tr>
									...
									<td class="form-editor">
										<input id="messagetype" name="messagetype" class="easyui-combobox"
											   data-options="panelHeight:'auto', valueField:'id',textField:'text',data:[{id: 1,text: '游戏'},{id: 2,text: '房间'},{id: 3,text: '全部'}]" />
									</td>
									...
									<td class="form-editor">
										<input id="nullity" name="nullity" class="easyui-combobox"
											   data-options="panelHeight:'auto', valueField:'id',textField:'text',data:[{id: 0,text: '启用'},{id: 1,text: '禁用'}]" />
									</td>
					...
					<div id="tb" style="display:none" class="opt-button-float">
						<div class="group-button opt-button-float">
							...
							<button class="layui-btn layui-btn-sm layui-btn-warm" onclick="batchTransfer(0)">
								<i class="iconfont" data-icon="&#xe649;">&#xe649;</i><span>批量启用</span>
							</button>
							<button class="layui-btn layui-btn-sm layui-btn-warm" onclick="batchTransfer(1)">
								<i class="iconfont" data-icon="&#xe649;">&#xe649;</i><span>批量禁用</span>
							</button>
						</div>
					</div>

					<!-- datagrid1 queryParams: {isLocked : 0,isDeleted : 0}, -->
					<table id="datagrid1" class="easyui-datagrid" title="系统消息"
						...
						<thead>
							<tr>
								...
								<th data-options="field:'starttime',align:'center',fixed:false,width:30,formatter:function(value,row,index){return $.dateFormatter(value)}">开始时间</th>
								<th data-options="field:'concludetime',align:'center',fixed:false,width:30,formatter:function(value,row,index){return $.dateFormatter(value)}">结束时间</th>
								<th data-options="field:'messagetypeDesc',align:'center',fixed:false,width:30">消息类型</th>
								<th data-options="field:'messagestring',align:'center',fixed:false,width:30">消息内容</th>
								<th data-options="field:'timerate',align:'center',fixed:false,width:30">频率(秒)</th>
	...
	<script type="text/javascript">
		...
		$(function($){
			...
			$("#add").bind("click",function(){
				$('#dialog').dialog({
					title: '新增系统消息',
					href: '${pageContext.request.contextPath}/uphold/sysmsg/toSysmsgAddPage',
					width: 800,
					height: 400,
					closed: false,
					cache: false,
		...
		function search(){
			...
			$('#datagrid1').datagrid({
				url: '${pageContext.request.contextPath}/uphold/sysmsg/querySysmsgWithPage',
				queryParams : {
		...
		function openEditWindown(id){
			$('#dialog').dialog({
				title: '修改任务',
				href: '${pageContext.request.contextPath}/uphold/sysmsg/toSysmsgEditPage',
				dialogParams: {id: id},
				width: 800,
				height: 400,
				closed: false,
		...
		function removeByids(){
			...
			$.messager.confirm('确认',"确定要删除选中的记录吗",function(r){
				if (r){
					...
					//提交到后台
					$.ajax({
						url: "${pageContext.request.contextPath}/uphold/sysmsg/removeSysmsgByIds?ids="+nmArr,
						type: 'DELETE',
						//dataType: 'json',
						//data: data,
		...
		function batchTransfer(sing){
			...
			$.messager.confirm('确认',"确定要操作选中的记录吗",function(r){
				if (r){
					...
					//提交到后台
					$.ajax({
						url: "${pageContext.request.contextPath}/uphold/sysmsg/batchTransfer",
						type: 'PUT',
						dataType: 'json',
						data: data,
						success: function(resp){
