...
<body>
	...
	<script type="text/javascript">
		...
		//ajax 加载form表单数据
		$.ajax({
			url: "${pageContext.request.contextPath}/uphold/spread/querySpreadTypeById",
			type: 'GET',
			dataType: 'json',
			data: {id: param.id},
			success: function(resp){
				...
				if(resp.success){
					...
					$('#taskedit-form').form("load",{
						id: resp.data.id,
						gradeid: resp.data.gradeid,
						percentage: resp.data.percentage,
						teamkpiend: resp.data.teamkpiend,
						teamkpistart: resp.data.teamkpistart
		...
		$("#save1").bind("click",function(){
           ...
           // console.log(obj);
			...
			//提交到后台
			$.ajax({
				url: "${pageContext.request.contextPath}/uphold/spread/updateSpreadType",
                type: 'PUT',
                dataType: 'json',
                data: data,
                success: function(resp){
